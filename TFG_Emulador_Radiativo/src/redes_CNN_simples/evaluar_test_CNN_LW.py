...
MODELO_PATH = "modelo_climart_cnn_LW.keras" # Apuntamos a la nueva CNN de Onda Larga
...
print("\nCargando la CNN y los datos de Test...")
...
# --- GENERACIÓN DE GRÁFICAS (ESCALA LINEAL - ONDA LARGA) ---
print("\nGenerando gráficas térmicas de evaluación final para la CNN...")

# 1. Gráfica de Dispersión (Real vs Predicho)
plt.figure(figsize=(8, 8))
# Vuelve el bins='log' para arreglar el contraste de color
plt.hexbin(Y_test_flat, Y_pred_flat, gridsize=50, cmap='Reds', mincnt=1, bins='log')
plt.colorbar(label='Densidad de puntos (Color en Escala Logarítmica)')
...
plt.title('CNN Onda Larga: Valores Reales vs. Predicciones', fontsize=14)
plt.xlabel('Flujo Radiativo Real Térmico (W/m²)', fontsize=12)
plt.ylabel('Flujo Radiativo Predicho (W/m²)', fontsize=12)
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('test_cnn_LW_scatter.png', dpi=300, bbox_inches='tight')
plt.close()
...
plt.title('CNN Onda Larga: Distribución de Errores - Test 2007', fontsize=14)
plt.xlabel('Error (Predicho - Real) [W/m²]', fontsize=12)
plt.ylabel('Frecuencia (Escala Lineal)', fontsize=12)
plt.grid(True, alpha=0.3)

plt.text(0.05, 0.95, f'RMSE Final CNN (LW): {rmse_final:.2f} W/m²', 
         transform=plt.gca().transAxes, verticalalignment='top', horizontalalignment='left',
         bbox=dict(boxstyle='round', facecolor='white', alpha=0.9))

plt.savefig('test_cnn_LW_histograma.png', dpi=300, bbox_inches='tight')
plt.close()

print(f"\n" + "="*50)
print(f"¡EVALUACIÓN CNN ONDA LARGA COMPLETADA!")
print(f"RMSE Anterior (MLP Reg): 23.39 W/m²")
