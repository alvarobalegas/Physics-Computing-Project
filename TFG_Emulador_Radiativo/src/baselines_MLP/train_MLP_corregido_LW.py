...
print("\nIniciando entrenamiento MLP REGULARIZADO (Onda Larga)...")
history_reg = modelo_reg.fit(
    train_dataset, 
    validation_data=val_dataset, 
    epochs=50, 
    callbacks=[early_stopping], # Activamos la parada temprana
    verbose=1
)

# --- GUARDADO ---
modelo_reg.save("modelo_climart_mlp_reg_LW.keras")
historia = history_reg.history

plt.figure(figsize=(10, 6))
plt.plot(historia['rmse'], label='Entrenamiento (1990)', color='blue', linewidth=2)
plt.plot(historia['val_rmse'], label='Validación (2005)', color='green', linewidth=2)
plt.title('MLP Regularizado Onda Larga: Evolución del RMSE', fontsize=14)
plt.xlabel('Épocas', fontsize=12)
plt.ylabel('RMSE (W/m²)', fontsize=12)
plt.legend()
plt.grid(True, linestyle='--', alpha=0.7)
plt.savefig('curva_rmse_mlp_reg_LW.png', dpi=300, bbox_inches='tight')
plt.close()
print("Entrenamiento Regularizado finalizado. Revisa 'curva_rmse_mlp_reg_LW.png'.")